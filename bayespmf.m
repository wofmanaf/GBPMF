...
for epoch = epoch:maxepoch
  ...
  %alpha_m = wishrnd(WI_post,df_mpost)*((gammaF_xkp(0.5).^2)/gammaF_xkp(0.75)/gammaF_xkp(0.25));   
  ...
  %alpha_u = wishrnd(WI_post,df_mpost)*((gammaF_xkp(0.5).^2)/gammaF_xkp(0.75)/gammaF_xkp(0.25));
  ...
  for gibbs=1:2 
   ...
   temp = abs((ratings_test - probe_rat_all));
   %err = sqrt( sum(temp)/pairs_pr);
   err = sum(temp)/pairs_pr;

   iter=iter+1;
   overall_err(iter)=err;

  %fprintf(1, '\nEpoch %d \t Average Test RMSE %6.4f \n', epoch, err);
  fprintf(1, '\nEpoch %d \t Average Test MAE %6.4f \n', epoch, err);
