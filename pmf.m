...
if restart==1 
  ...
  num_feat = 20; % Rank 10 decomposition 

  w1_M1     = 0.1*randn(num_m, num_feat); % Movie feature vectors
  w1_P1     = 0.1*randn(num_p, num_feat); % User feature vecators
  %x1=rand(num_m*num_feat,1);
  %m1 = GGD_xkp(x1,4,sqrt(gammaF_xkp(0.25)/gammaF_xkp(0.75)));
  %w1_M1 = 0.1*reshape(m1,num_m,num_feat);
  %x2=rand(num_p*num_feat,1);
  %m2 = GGD_xkp(x2,4,sqrt(gammaF_xkp(0.25)/gammaF_xkp(0.75)));
  %w1_P1 = 0.1*reshape(m2,num_p,num_feat);
...
for epoch = epoch:maxepoch
  ...
  for batch = 1:numbatches
    ...
    f = sum( abs(pred_out - rating) + ...
        0.5*lambda*( sum( abs((w1_M1(aa_m,:)) + abs(w1_P1(aa_p,:))),2)));
  ...
  %f_s = sum( (pred_out - rating).^2 + ...
        %0.5*lambda*( sum( (w1_M1(aa_m,:).^2 + w1_P1(aa_p,:).^2),2)));
  f_s = sum( abs((pred_out - rating)) + ...
        0.5*lambda*( sum( abs((w1_M1(aa_m,:)) + abs(w1_P1(aa_p,:))),2)));  
     
  %err_train(epoch) = sqrt(f_s/N);
  err_train(epoch) = f_s/N;
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  %err_valid(epoch) = sqrt(sum(abs((pred_out- rating)))/NN);
  err_valid(epoch) = sum(abs((pred_out- rating)))/NN;
  fprintf(1, 'epoch %4i batch %4i Training RMSE %6.4f  Test MAE %6.4f  \n', ...
              epoch, batch, err_train(epoch), err_valid(epoch));
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
